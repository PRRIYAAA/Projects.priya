...
with sr.Microphone() as source:
    print('Do you want to read mail or send mail...')
    speak('Do you want to read mail or send mail...')
    ...
    if 'send' in d:
        try:
            ...
            while loop=='yes':
                ...
                speak('Your mail subject: {}'.format(t))
                text=recognizer.recognize_google(recordedaudio, language="en-US")
                print('Your message: {}'.format(text))
                speak('Your message: {}'.format(text))
                
                with sr.Microphone() as source:
                    print('Is there any changes you want to make')
                    speak('Is there any changes you want to make')
                    check=recognizer.listen(source,10)
                loop=recognizer.recognize_google(check, language="en-US")
                print('Your said: {}'.format(loop))
                if 'no' in loop:
                    loop='no'
                elif 'yes' in loop:
                    loop='yes'
        ...
        def read_email_from_gmail():
            try:
                ...
                speak('Do you want to read the recent messages or from an individual?')
                with sr.Microphone() as source:
                    recognizer.adjust_for_ambient_noise (source, duration=1)
                ...
                if 'recent' in re:
                    ...
                    speak('Whos mail do you want me to read?...')
                    with sr.Microphone() as source:
                        recognizer.adjust_for_ambient_noise (source, duration=1)
                ...
                for i in range(len(msgs)-1,0,-1):
                    ...
                    for response_part in msgs[i]:
                        if type(response_part) is tuple:
                            ...
                            if f==0:
                                speak('Should I reed this message from'+email_from)
                            else:
                                speak('You have recieved an email from'+email_from+'Should I reed?')
                            with sr.Microphone() as source:
                                recognizer.adjust_for_ambient_noise (source, duration=1)
                            ...
                            try:
                                ...
                                if 'yes' in h:
                                    ...
                                    speak('The subject is'+str(email_subject))
                                    print('Body :' + email_body + '\n')
                                    speak('The message is:'+email_body)
                            except Exception as ex:
                                print(ex)
                        ...
                        if isinstance(arr, tuple):
                            ...
                            try:
                                ...
                                if 'yes' in h:
                                    ...
                                    #speak('The message is'+str(message))
                                    print("-"*100)
                            except Exception as ex:
                                print(ex)
                    if index==2:
